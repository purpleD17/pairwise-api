  ...
  module Routing
    class Segment #:nodoc:
      ...
      def interpolation_chunk
        RailsLts::Support::URI.escape_unsafe_characters(value, UNSAFE_PCHAR)
      end
    ...
    class DynamicSegment < Segment #:nodoc:
      ...
      def interpolation_chunk(value_code = local_name)
        "\#{RailsLts::Support::URI.escape_unsafe_characters(#{value_code}.to_s, ActionController::Routing::Segment::UNSAFE_PCHAR)}"
      end
      ...
      def match_extraction(next_capture)
        ...
        %[
          value = if (m = match[#{next_capture}])
            RailsLts::Support::URI.unescape_uri(m)
          else
            #{default_value}
          end
    ...
    class PathSegment < DynamicSegment #:nodoc:
      ...
      def extract_value
        "#{local_name} = hash[:#{key}] && Array(hash[:#{key}]).collect { |path_component| RailsLts::Support::URI.escape_unsafe_characters(path_component.to_param, ActionController::Routing::Segment::UNSAFE_PCHAR) }.to_param #{"|| #{default.inspect}" if default}"
      end
      ...
      class Result < ::Array #:nodoc:
        ...
        def self.new_escaped(strings)
          new strings.collect {|str| RailsLts::Support::URI.unescape_uri str}
        end
      end
    end
      ...
      def match_extraction(next_capture)
        %[
          if (m = match[#{next_capture}])
            params[:#{key}] = RailsLts::Support::URI.unescape_uri(m.from(1))
          end
        ]
      end
    end
